...
public class Auto_SingleCam_ShootWhileMoving_RR extends LinearOpMode {
    ...
    private enum Pipeline { TAG, BALL }
    private Pipeline activePipeline = Pipeline.BALL;
    private long lastPipelineSwitchMs = 0;
    private static final long PIPELINE_SWITCH_MS = 160;
    private long lastTagCheckMs = 0;
    private static final long TAG_CHECK_INTERVAL_MS = 400;
    ...
    public void runOpMode() throws InterruptedException {
        ...
        while (!isStarted() && !isStopRequested()) {
            ...
            if (activePipeline == Pipeline.TAG) {
                ...
                if (ballPipeline != null && ballPipeline.isBallFound())
                    telemetry.addData("Ball", "xerr=%.1f y=%.1f", ballPipeline.getBallXError(), ballPipeline.getBallY());
        ...
        while (opModeIsActive() && runtime.seconds() < 29.5) {
            drive.update();
            maybePeriodicTagCheck();
            switch (state) {
                case SEARCHING:
                    setFlywheelRPM(currentTargetRPM);
                    ensurePipeline(Pipeline.BALL);
                    if (ballPipeline != null && ballPipeline.isBallFound()) {
                        double bx = ballPipeline.getBallX();
                        double by = ballPipeline.getBallY();
                        if (bestCandidateY < 0) { bestCandidateY = by; bestCandidateX = bx; candidateTimer.reset(); }
                        else {
                        ...
                        } else drive.setWeightedDrivePower(new Pose2d(0,0,0.20));
                    } else {
    ...
    private void maybeEnsurePipelineForState() {
    }
    ...
    private void initHardware() {
        ...
        servo1 = hardwareMap.get(Servo.class, "servo1"); //left
        servo2 = hardwareMap.get(Servo.class, "servo2"); //right
    ...
    private void initSingleCamera() {
        ...
        camera.openCameraDeviceAsync(new OpenCvCamera.AsyncCameraOpenListener() {
            @Override public void onOpened() {
                ...
                activePipeline = Pipeline.BALL;
                lastPipelineSwitchMs = System.currentTimeMillis();
                lastTagCheckMs = System.currentTimeMillis();
            }
            @Override public void onError(int errorCode) {}
        });
    }

    private void ensurePipeline(Pipeline p) {
        long now = System.currentTimeMillis();
        if (activePipeline == p) return;
        if (now - lastPipelineSwitchMs < PIPELINE_SWITCH_MS) return;
        if (p == Pipeline.TAG) camera.setPipeline(tagPipeline);
        else camera.setPipeline(ballPipeline);
        activePipeline = p;
        lastPipelineSwitchMs = now;
    }

    private void maybePeriodicTagCheck() {
        long now = System.currentTimeMillis();
        if (now - lastTagCheckMs > TAG_CHECK_INTERVAL_MS) {
            ensurePipeline(Pipeline.TAG);
            ArrayList<AprilTagDetection> tags = tagPipeline.getLatestDetections();
            if (tags != null && tags.size() > 0) {
                for (AprilTagDetection t : tags) {
                    if (FIELD_TAG_POSES.containsKey(t.id)) {
                        ...
                        if (poseFromTag != null) drive.setPoseEstimate(new Pose2d(poseFromTag[0], poseFromTag[1], poseFromTag[2]));
                        break;
                    }
                }
            }
