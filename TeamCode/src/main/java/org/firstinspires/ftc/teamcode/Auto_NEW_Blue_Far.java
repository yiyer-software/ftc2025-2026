...
@Autonomous(name = "Auto_NEW_Blue_Far", group = "Auton")
public class Auto_NEW_Blue_Far extends LinearOpMode {
    ...
    private static final double SORT_DEGREES          = 171.0;
    ...
    public void runOpMode() throws InterruptedException {
        ...
        MecanumDrive drive = new MecanumDrive(hardwareMap, new Pose2d(56, -20, Math.toRadians(-165)));

        initHardware();
        waitForStart();
        if (isStopRequested()) return;

        Action lookMotif = drive.actionBuilder(new Pose2d(56, -20, Math.toRadians(-165)))
                .turnTo(Math.toRadians(170))
                .build();

        Action driveToFirstPickup = drive.actionBuilder(new Pose2d(56, -20, Math.toRadians(-165)))
                .splineToLinearHeading(new Pose2d(36, -30, Math.toRadians(-90)), Math.toRadians(-90))
                .strafeTo(new Vector2d(36, -50))
                .build();

        Action driveToShoot1st = drive.actionBuilder(new Pose2d(36, -50, Math.toRadians(-90)))
                .splineToLinearHeading(new Pose2d(56, -20, Math.toRadians(-165)), Math.toRadians(0))
                .build();

        Action driveToSecondPickup = drive.actionBuilder(new Pose2d(56, -20, Math.toRadians(-165)))
                .splineToLinearHeading(new Pose2d(-12, -30, Math.toRadians(-90)), Math.toRadians(-90))
                .strafeTo(new Vector2d(-12, -48))
                .build();

        Action driveToShoot2nd = drive.actionBuilder(new Pose2d(-12, -48, Math.toRadians(-90)))
                .splineToLinearHeading(new Pose2d(-34, -34, Math.toRadians(-135)), Math.toRadians(-180))
                .build();

        Action driveToThirdPickup = drive.actionBuilder(new Pose2d(-34, -34, Math.toRadians(-135)))
                .splineToLinearHeading(new Pose2d(12, -30, Math.toRadians(-90)), Math.toRadians(-90))
                .strafeTo(new Vector2d(12, -48))
                .build();

        Action driveToShoot3rd = drive.actionBuilder(new Pose2d(12, -48, Math.toRadians(-90)))
    ...
    private Action intakeAction() {
        return new Action() {
            ...
            public boolean run(@NonNull TelemetryPacket packet) {
                ...
                if (Actions.now() - cycleStart >= 7.0 && intakeState != 2) {
