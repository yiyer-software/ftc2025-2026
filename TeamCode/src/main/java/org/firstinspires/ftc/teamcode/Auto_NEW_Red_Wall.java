...
@Autonomous(name = "Auto_NEW_Red_Wall", group = "Auton")
public class Auto_NEW_Red_Wall extends LinearOpMode {
    ...
    private static final double SORT_POWER            = 0.5;
    ...
    public void runOpMode() throws InterruptedException {
        ...
        MecanumDrive drive = new MecanumDrive(hardwareMap, new Pose2d(-52, 48, Math.toRadians(135)));
        ...
        Action driveToFirstCheckpt = drive.actionBuilder(new Pose2d(-52, 48, Math.toRadians(135)))
                .strafeTo(new Vector2d(-36, 34))
                .build();

        Action driveToFirstHalfCheckpt = drive.actionBuilder(new Pose2d(-36, 34, Math.toRadians(135)))
                .strafeTo(new Vector2d(-12, 18))
                .turnTo(Math.toRadians(180))
                .build();

        Action driveToFirstPickup = drive.actionBuilder(new Pose2d(-12, 18, Math.toRadians(180)))
                .turnTo(Math.toRadians(90))
                .strafeTo(new Vector2d(-12, 50))
                .build();

        Action driveToShoot1st = drive.actionBuilder(new Pose2d(-12, 50, Math.toRadians(90)))
        ...
        Action driveToSecondPickup = drive.actionBuilder(new Pose2d(-45, 34, Math.toRadians(90)))
        ...
        Action driveToShoot2nd = drive.actionBuilder(new Pose2d(12, 50, Math.toRadians(90)))
                .strafeTo(new Vector2d(12, 44))
                .splineToLinearHeading(new Pose2d(-30, 32, Math.toRadians(135)), Math.toRadians(0))
                .build();

        Action driveToThirdPickup = drive.actionBuilder(new Pose2d(-30, 32, Math.toRadians(135)))
                .splineToLinearHeading(new Pose2d(36, 20, Math.toRadians(90)), Math.toRadians(0))
                .strafeTo(new Vector2d(36, 50))
                .build();

        Action driveToShoot3rd = drive.actionBuilder(new Pose2d(36, 50, Math.toRadians(90)))
        ...
        return new Action() {
            ...
            public boolean run(@NonNull TelemetryPacket packet) {
                ...
                if (!initialized) {
                    ...
                    shootState  = 0;
                    initialized = true;
                    // FIX: Start flywheels as soon as shootingAction begins
                    double ticksPerSec = (3000.0 / 60.0) * TICKS_PER_REV;
                    launcherLeft.setVelocity(ticksPerSec);
                    launcherRight.setVelocity(ticksPerSec);
                }

                // FIX: Keep flywheel velocity command active every loop so PID holds speed
                double ticksPerSec = (3000.0 / 60.0) * TICKS_PER_REV;
                launcherLeft.setVelocity(ticksPerSec);
                launcherRight.setVelocity(ticksPerSec);
                ...
                switch (shootState) {
                    ...
                    case 0:
                        if (!sortMotor.isBusy() && ballsIn[0] > 0) {
                    ...
                    case 4:
                        if (ballsIn[0] <= 0 || shotsFired >= 3) {
                            shootState = 5;
                        } else if (Actions.now() - liftTimer >= 0.75) {
                            shootState = 0;
                        }
                        break;
    ...
    private Action intakeAction() {
        return new Action() {
            ...
            public boolean run(@NonNull TelemetryPacket packet) {
                ...
                // FIX: Only time out when actively waiting for a ball (state 0).
                // States 1 and 2 are mid-detection or mid-sort — let them finish.
                if (Actions.now() - cycleStart >= 7.0 && intakeState == 0) {
                    ...
                    packet.put("intakeState", "Timed out");
                    packet.put("ballsIn",     ballsIn[0]);
                    return false;
                }
                ...
                packet.put("ballsIn",     ballsIn[0]);
                packet.put("timeElapsed", Actions.now() - cycleStart);
                return true;
            }
        };
    }
        ...
        return new Action() {
            ...
            public boolean run(@NonNull TelemetryPacket packet) {
                ...
                if (MotifStorage.motifDetected) {
                    packet.put("Motif_Tag",    MotifStorage.motifTagId);
                    packet.put("Motif_Status", "Already locked");
                    return false;
                }

                boolean found = limelight.identifyMotifTag();
                packet.put("Motif_Found",   found);
                packet.put("Motif_Elapsed", Actions.now() - startTime);

                if (found) {
                    packet.put("Motif_Tag",    MotifStorage.motifTagId);
                    packet.put("Motif_Status", "Locked in");
                    return false;
                }

                if (Actions.now() - startTime >= MOTIF_TIMEOUT_SECS) {
                    packet.put("Motif_Status", "Timed out");
                    return false;
                }
            ...
            public boolean run(@NonNull TelemetryPacket packet) {
                ...
                if (tagData != null && tagData.poseValid && tagData.tagVisible) {
                    ...
                    Pose2d limelightPose = new Pose2d(
                            tagData.x,
                            tagData.y,
                            Math.toRadians(tagData.yaw)
                    );
                    drive.localizer.setPose(limelightPose);
                    packet.put("LL_X",       tagData.x);
                    packet.put("LL_Y",       tagData.y);
                    packet.put("LL_Yaw",     tagData.yaw);
                    packet.put("LL_TagID",   tagData.tagId);
                    packet.put("LL_Samples", samplesCollected);
                } else {
                    packet.put("LL_Status", "No tag visible");
                }
